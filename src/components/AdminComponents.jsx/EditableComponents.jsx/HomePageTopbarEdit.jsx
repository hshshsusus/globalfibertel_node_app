...
const HomePageTopbarEdit = () => {
    ...
    const handleSave = async (id) => {
        try {
            const updatedLocation = locations.find((item) => item?.id === id);
            const res = await axios.put(BASE_URL + "/admin/homepage/locations/edit/" + id, updatedLocation, { withCredentials: true });
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {Object.keys(location).map((field) => {
                                ...
                                const isTextArea = field === "location" || field === "timings" || field === "customerCareNumber";
                                const isHiddenField = field === "id"

                                if (isHiddenField) return null;

                                return (
                                    <div key={field} className="flex flex-col gap-1">
                                        {location[field] !== null && <label className="text-sm font-semibold text-gray-600">{field}</label>}
                        ...
                        <div className="flex justify-end gap-3 mt-6">
                            {editingId === location?.id ? (
                                ...
                                >
                                    Edit
                                    <FaRegEdit className="font-bold text-[18px]" />
                                </button>
                            )}
                        </div>
                    </div>
...
export default HomePageTopbarEdit;
