...
const AddPack = () => {
    ...
    const addPack = useSelector(store => store.pack.addPack);
    ...
    const handleNewPack = async () => {
        setLoad(true);
        try {
            const res = await axios.post(BASE_URL + "/packages/add", {
                upload,
                download,
                validity,
                price
    ...
    return (
        <>
            <div className="flex gap-1.5 items-center justify-center border-t border-gray-300 py-5">
                ...
                <LuPackagePlus className="text-[30px] text-orange-500 font-bold" />
            </div>
            <form class="max-w-sm mx-auto mt-[25px] add py-10 px-6 rounded-md" onSubmit={(e) => e.preventDefault()}>
                ...
                <button type="submit" class="text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-blue-300 font-medium rounded-tl-[10px] rounded-br-[10px] text-sm px-5 py-2.5 text-center cursor-pointer" onClick={handleNewPack}>{load ? <Loader /> : "Add pack"}</button>
            </form>
        </>
    )
}
